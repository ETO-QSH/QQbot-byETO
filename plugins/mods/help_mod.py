...
from plugins.deepseek import ai_fallback


help_ai = on_command("帮助", rule=to_me(), aliases={"-h", "help"})

folder = Path("plugins\\mods")
data = {file.name: file.read_text(encoding='utf-8') for file in folder.iterdir() if file.is_file()}
help_meta = f'''
...
项目的管理者为 {config.superusers}，如有需求和或者bug都可以反馈
...
@help_ai.handle()
async def help_eto(bot: Bot, event: Event, matcher: Matcher):
    try:
        msg = await ai_fallback(event.get_user_id(), str(event.get_message()), help_meta, data)
        if msg == "请求失败：Connection error.":
            await help_ai.finish(MessageSegment.at(event.get_user_id()), "deepseek链接失败喵，大概率的没钱买api了说")
    except Exception as e:
        await help_ai.finish(MessageSegment.at(event.get_user_id()) + help_meta)
